    ...
    <div class="room-detail-hero-inner">
        <div class="room-detail-copy">
            ...
            <div class="room-detail-actions">
                ...
                <a href="{{ auth('guest')->check() ? route('reservation') : '#guest-auth-modal' }}" class="btn room-detail-primary{{ auth('guest')->check() ? '' : ' js-auth-trigger' }}"{{ auth('guest')->check() ? '' : ' data-auth-trigger' }}>Book This Room</a>
            </div>
        </div>
