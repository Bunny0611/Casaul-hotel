...
<div class="assigned-page space-y-6">
    ...
    <div class="task-panel">
        ...
        <div class="overflow-x-auto"><table class="task-table min-w-[980px] w-full text-left text-sm"><thead class="text-xs uppercase text-gray-500"><tr><th class="p-4">Room</th><th class="p-4">Room Type</th><th class="p-4">Task</th><th class="p-4">Priority</th><th class="p-4">Assigned Staff</th><th class="p-4">Scheduled Date</th><th class="p-4">Scheduled Time</th><th class="p-4">Started At</th><th class="p-4">Finished At</th><th class="p-4">Status</th><th class="p-4">Action</th></tr></thead>
        <tbody class="divide-y divide-gray-100">@forelse($tasks as $task)<tr>
            <td class="p-4 font-semibold">{{ $task->room->room_number }}</td><td class="p-4">{{ $task->room->room_type }}</td><td class="p-4">{{ $task->task }}</td><td class="p-4">{{ ucfirst($task->priority) }}</td><td class="p-4">{{ $task->assignedStaff?->name ?? 'Unassigned' }}</td><td class="p-4">{{ $task->scheduled_date->format('M d, Y') }}</td><td class="p-4">{{ $task->scheduled_time ? \Carbon\Carbon::parse($task->scheduled_time)->format('g:i A') : 'N/A' }}</td><td class="p-4">{{ $task->started_at?->format('M d, Y g:i A') ?? 'N/A' }}</td><td class="p-4">{{ $task->finished_at?->format('M d, Y g:i A') ?? 'N/A' }}</td><td class="p-4"><span class="status-badge status-{{ $task->status }}">{{ ucfirst(str_replace('_', ' ', $task->status)) }}</span></td><td class="p-4"><div class="flex flex-wrap gap-2"><button type="button" class="rounded bg-gray-700 px-3 py-1 text-white" data-task-details="{{ e(json_encode(['occupancy' => $task->reservation?->number_of_guests ?? 'N/A', 'contact' => $task->assignedStaff?->contact_no ?? 'N/A', 'reference' => $task->reservation ? 'RES-'.$task->reservation->id : 'N/A', 'duration' => $task->estimated_duration ? $task->estimated_duration.' minutes' : 'N/A', 'notes' => $task->notes ?? 'N/A'])) }}">View</button>@if($task->status === 'pending')<form method="POST" action="{{ route('housekeeping.tasks.start', $task) }}">@csrf @method('PATCH')<button class="rounded bg-blue-600 px-3 py-1 text-white">Start</button></form><form method="POST" action="{{ route('housekeeping.tasks.destroy', $task) }}" onsubmit="return confirm('Delete this pending task?')">@csrf @method('DELETE')<button class="rounded bg-red-600 px-3 py-1 text-white">Delete</button></form>@else<form method="POST" action="{{ route('housekeeping.tasks.complete', $task) }}">@csrf @method('PATCH')<button class="rounded bg-green-600 px-3 py-1 text-white">Complete</button></form>@endif</div></td>
        </tr>@empty<tr><td colspan="11" class="p-10 text-center text-gray-500">No assigned cleaning tasks.</td></tr>@endforelse</tbody></table></div>
    </div>
</div>
