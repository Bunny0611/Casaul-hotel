@extends($routePrefix === 'employee.' ? 'employee.layout' : 'admin.layout')
...
<div class="space-y-6">
    <div class="flex flex-col gap-3 rounded-2xl bg-white p-6 shadow-sm sm:flex-row sm:items-center sm:justify-between">
        ...
        <a href="{{ route($routePrefix . ($routePrefix === 'employee.' ? 'reservation' : 'reservations')) }}" class="rounded-lg border border-gray-300 px-4 py-2 text-sm font-semibold text-gray-700 hover:bg-gray-50">Back to Reservations</a>
    </div>
                    ...
                    <tr>
                        ...
                        <td class="px-4 py-4">
                            @if($refund->status === 'Pending')
                                <form method="POST" action="{{ route($routePrefix . 'refunds.mark-refunded', $refund) }}">
                                    @csrf
                                    @method('PATCH')
